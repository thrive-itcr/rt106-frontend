...
//		console.log( strips );
		...
		if (canvas.getContext) {
            ...
            var pixelData = new Uint16Array(imageWidth * imageLength * 4);
				...
				for (var y = 0, j = 0; y < numRowsInStrip, j < numPixels; y++) {
					...
					for (var x = 0; x < imageWidth; x++, j++) {
						...
						switch (photometricInterpretation) {
							...
							// BlackIsZero
							case 1:
								red = green = blue = pixelSamples[0], sampleProperties[0].bitsPerSample;
							break;

							// RGB Full Color
							case 2:
								red = pixelSamples[0], sampleProperties[0].bitsPerSample;
								green = pixelSamples[1], sampleProperties[1].bitsPerSample;
								blue = pixelSamples[2], sampleProperties[2].bitsPerSample;
							break;
							...
							case 3:
								...
								red = colorMapValues[colorMapIndex], 16;
								green = colorMapValues[colorMapSampleSize + colorMapIndex], 16;
								blue = colorMapValues[(2 * colorMapSampleSize) + colorMapIndex], 16;
							break;
                        ...
                        pixelData[4 * ((yPadding + y) * imageWidth + x) + 0] = red;
                        pixelData[4 * ((yPadding + y) * imageWidth + x) + 1] = blue;
                        pixelData[4 * ((yPadding + y) * imageWidth + x) + 2] = green;
                        pixelData[4 * ((yPadding + y) * imageWidth + x) + 3] = opacity * 255;
					}
				}
...
/*		for (var i = 0, numFileDirectories = this.fileDirectories.length; i < numFileDirectories; i++) {
        ...
        return {pixelData: pixelData, width: imageWidth, height: imageLength};
		// return this.canvas;
	},
}
